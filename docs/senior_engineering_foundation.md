...
- **Tenant scoping is resolved per door,** each from its own credential (owner JWT → `app_metadata`, visitor key → key row, MCP token → token claim), into a typed context in `tenants/contexts.py`. Isolation is enforced at each door's credential boundary, never from request input, and never through a single cross-door dependency (see ADR-0006, which superseded the original single `get_current_tenant` seam). This is the seam the Day-2 test proves.
