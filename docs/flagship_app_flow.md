...
1. The widget POSTs the visitor's question to the API with the tenant's public widget key. There is NO visitor login and NO user JWT on this path. The API is a Backend-for-Frontend (BFF): the model API key never reaches the browser; the bundle holds only the public widget key. The public widget key is an identifier, not a secret (it sits in page source), so the visitor edge gate validates it in layers: the key exists in a valid-keys store, the request Origin/Referer is on that key's allowed-domain allowlist, a per-key/per-IP rate limit applies, then the request is sanitized down to just the query. Origin/Referer binding stops casual browser copy-paste theft but is spoofable by a non-browser client, so the load-bearing controls are the narrow scope (tenant-grounded answers only, capped output), the rate limit, and grounding plus injection screening. The Origin allowlist is built simply and treated as a deterrent, not the main defense. The gate lives as FastAPI middleware (see open decisions).
...
4. The agent's `retrieve` tool runs a hybrid search inside that tenant only. Tenant isolation is enforced by the Weaviate shard boundary and by RLS in the relational layer. The gate calls the shared core capabilities (`retrieve`, `answer`-with-verdict) DIRECTLY in-process via a thin adapter. It does NOT route the visitor request through the MCP (no self-network-hop). The MCP is a separate external door over the same core.
...
2. It is Origin-validated and verifies an **external-caller token** (an OAuth token or scoped API key that carries/binds the tenant), **NOT the owner's Supabase login JWT**. This is a separate external door for external AI callers (for example a customer wiring the assistant into ChatGPT or their own agent), distinct from the owner login and from the widget path. The server acts as an OAuth 2.1 Resource Server: it validates signature, issuer, audience (equal to this server, per RFC 8707), and expiry, rejects any token not issued for this server, validates the Origin header (403 on present-and-invalid, the DNS-rebinding defense), and never passes the inbound token upstream.
3. It exposes the answer-plus-verdict capability as an MCP tool via a thin adapter over the shared core function (the same core the widget calls in-process). The honesty verdict is FUSED into `answer` (every call returns {answer, verdict}); it is never an optional tool the caller can skip, because honesty is the product. A standalone general-purpose evaluator tool (grade arbitrary caller-supplied text against sources) MAY additionally be exposed and is kept distinct from the fused verdict and from the offline DeepEval CI gate.
...
- Edge/app split: REOPENED then resolved. The visitor edge gate lives as FastAPI middleware (Option B) for finishability and testability on a 12-day private demo. A thin Cloudflare Worker plus WAF rate-limiting front (Option A) is the documented production move and an optional Day-11-buffer stretch if Cloudflare is specifically wanted on the resume. Note: do not hand-roll rate-limit counters in Cloudflare KV (~1,000 writes/day, ~60s consistency); use the rate-limiting binding or WAF rate-limiting rules. Both options are free and card-free and neither touches Vertex/Gemini credits. **DECIDED: Option B (in-app FastAPI middleware), owner signed off 2026-06-30. Cloudflare is not forced into the request path now; it remains the documented production move and an optional later stretch, and is still the production scale-out target for the ingestion queue.**
