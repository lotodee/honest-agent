...
- **Update (visitor edge gate, owner re-vet):** the visitor gate (validate widget-key + Origin/Referer allowlist + rate limit + sanitize-to-query) lives as FastAPI middleware (Option B), not a Worker, for finishability and testability. The thin Cloudflare Worker + WAF rate-limiting front is the documented production move and an optional Day-11-buffer stretch. Do not hand-roll rate-limit counters in Cloudflare KV (~1,000 writes/day, ~60s consistency); use the rate-limiting binding or WAF rules. DECIDED: Option B (in-app FastAPI middleware), owner signed off 2026-06-30.
...
- **Final call: Alternative B. Ship a real remote (Streamable-HTTP) MCP server with Origin validation and a verified _external-caller_ token (an OAuth token or scoped API key that carries the tenant), NOT the owner's Supabase login JWT. The MCP is a separate door for external AI callers, a different trust domain from the owner login and from the visitor widget. Validate signature/issuer/audience (equal to this server, per RFC 8707)/expiry, reject tokens not issued for this server, and never pass the inbound token upstream. On Day 1 the demonstrated issuer is a self-minted scoped API key (free, in our control). Expose the answer-plus-verdict tool via a thin adapter over the shared core. Write the full OAuth 2.1 Resource Server design (Protected Resource Metadata RFC 9728, dynamic client registration, PKCE, no token passthrough, confused-deputy) into the README as "how this hardens for production," and demonstrate one piece of it (audience/issuer validation on the token).**
...
**MCP server:**
- A remote Streamable-HTTP MCP server on the Python service, single `/mcp` endpoint, Origin-validated, external-caller-token-verified (an OAuth token or scoped API key carrying the tenant, with signature/issuer/audience/expiry checked and no token passthrough; NOT the owner's Supabase login JWT), exposing the answer-plus-verdict tool via a thin adapter over the shared core. Full OAuth hardening is documented and one check (audience/issuer validation) is demonstrated.
...
Repo, tooling, CI skeleton, and the typed FastAPI surface are already done in Milestone 0 priming. Day 1 builds the auth spine: the three request paths across two trust domains, each resolving tenant from its own credential. (a) Owner Supabase JWT verify on the login/ingestion routes (the only real login), plus the Backend-for-Frontend secret guard. (b) The visitor widget-key gate as a first-class piece: key store + Origin/Referer allowlist + sanitize-to-query built fully, rate limit stubbed behind a clean interface. (c) The day-1 spike: stand up a remote Streamable-HTTP MCP server with Origin validation and a verified external-caller token (self-minted scoped API key on Day 1, sig/iss/aud/exp, no passthrough), confirm an MCP client can list and call a stub tool. Core capabilities (`retrieve`, `answer`-with-verdict) are plain functions behind two thin adapters (in-process for the widget, MCP tool for external callers), stubbed on Day 1. This proves the scoped MCP plan (decision 1.3) and the corrected three-path auth model work before anything depends on them. The visitor edge gate lives as in-app middleware (Option B), with a Cloudflare Worker front documented as the production move.
