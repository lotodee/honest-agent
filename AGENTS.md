...
- Tenant scoping is resolved per door, each from its own credential (see `docs/adr/0006-per-door-request-contexts.md`, the current decision, which supersedes ADR-0004's single-`get_current_tenant` wording): owner JWT → `app_metadata`, visitor key → key row, MCP token → token claim, each producing its own typed context in `tenants/contexts.py`. Keep one scoping seam WITHIN a door; do not re-introduce a single cross-door `get_current_tenant`. Never weaken a door's resolver per route. API schemas are separate from internal/DB models; every route declares `response_model`.
