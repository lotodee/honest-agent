...
- Run the fast quick-gate before committing: `ruff check`, `ruff format --check`, `mypy`, unit tests. Keep `main` green and deployable.
- Conventional Commits (`feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `ci`, ...). Trunk-based, short-lived branches, CI-gated PRs.
