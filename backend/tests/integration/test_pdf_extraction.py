...
def _write_text_pdf(path: Path) -> None:
    doc = fitz.open()
    page = doc.new_page()
    page.insert_text((72, 72), "Hello, this is a valid PDF with real native text.")
    doc.save(str(path))
    doc.close()
...
async def test_wellformed_pdf_extracts_native_text(tmp_path: Path) -> None:
    good = tmp_path / "good.pdf"
    _write_text_pdf(good)
    result = await extract_isolated(good)
    assert result.page_count == 1
    assert result.pages[0].source is PageSource.NATIVE
    assert "valid PDF" in result.pages[0].text


async def test_malformed_pdf_is_poison_and_parent_survives(tmp_path: Path) -> None:
...
async def test_timeout_is_reported_as_poison(tmp_path: Path) -> None:
    good = tmp_path / "good.pdf"
    _write_text_pdf(good)
    with pytest.raises(PoisonDocumentError) as caught:
